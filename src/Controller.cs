...
namespace AccStateSync
{
	public partial class AccStateSync
	{
		public partial class AccStateSyncController : CharaCustomFunctionController
		{
			...
			internal Dictionary<int, Dictionary<string, bool>> CharaVirtualGroupStates = new Dictionary<int, Dictionary<string, bool>>();
			...
			protected override void OnCoordinateBeingLoaded(ChaFileCoordinate coordinate)
			{
				...
				if (!MakerAPI.InsideMaker || (MakerAPI.InsideMaker && LoadCoordinateExtdata))
				{
					...
					if (ExtendedData?.version > Constants.ExtDataVersion)
						Logger.Log(BepInEx.Logging.LogLevel.Error | BepInEx.Logging.LogLevel.Message, $"[OnReload][{ChaControl.chaFile.parameter?.fullname}] ExtendedData.version: {ExtendedData.version} is newer than your plugin");
					if (ExtendedData != null && ExtendedData.data.TryGetValue("OutfitTriggerInfo", out var loadedOutfitTriggerInfo) && loadedOutfitTriggerInfo != null)
					{
						...
						Logger.Log(DebugLogLevel, $"[OnCoordinateBeingLoaded][{ChaControl.chaFile.parameter?.fullname}] CharaVirtualGroupInfo[{CurrentCoordinateIndex}] loaded from extdata");
					}
		...
		internal static AccStateSyncController GetController(ChaControl chaCtrl) => chaCtrl == null ? null : chaCtrl.gameObject.GetComponent<AccStateSyncController>();
	}
}
