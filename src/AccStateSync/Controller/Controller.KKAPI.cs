				...
				*/
				if (!JetPack.CharaMaker.Inside || (JetPack.CharaMaker.Inside && _loadCoordinateExtdata))
				{
					...
					if (_pluginData != null)
					{
						...
						//FillMissingGroup(_currentCoordinateIndex);
						/*
						if (!JetPack.CharaStudio.Running)
						{
							foreach (TriggerGroup _group in TriggerGroupList.Where(x=> x.Coordinate == _currentCoordinateIndex).ToList())
								_group.State = _group.Startup;
						}
						*/
						/*
				...
				if (!JetPack.CharaMaker.Inside || (JetPack.CharaMaker.Inside && _loadCharaExtdata))
				{
					...
					if (_pluginData != null)
					{
						...
						for (int _coordinate = 0; _coordinate < 7; _coordinate++)
							FillMissingGroup(_coordinate);
						*/
						/*
						if (!JetPack.CharaStudio.Running)
						{
							foreach (TriggerGroup _group in TriggerGroupList)
								_group.State = _group.Startup;
						}
						*/
						//RefreshCache();
